...
void sovet::propose( name proposer,
                        name proposal_name,
...
void sovet::approve( name proposer, name proposal_name, permission_level level,
...
void sovet::unapprove( name proposer, name proposal_name, permission_level level ) {
   require_auth( level );
...
void sovet::cancel( name proposer, name proposal_name, name canceler ) {
...
void sovet::exec( name proposer, name proposal_name, name executer ) {
   ...
   send_deferred( (uint128_t(proposer.value) << 64) | proposal_name.value, eosio::name{executer.value},
...
void sovet::invalidate( name account ) {
...
EOSIO_DISPATCH( eosio::sovet, (propose)(approve)(unapprove)(cancel)(exec)(invalidate) )
