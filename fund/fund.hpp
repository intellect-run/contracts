...
public:
  ...
  //атомарные транзакции фондового кошелька
    //паевый фонд
  [[eosio::action]] void addcirculate(eosio::name coopname, eosio::name contract, eosio::asset quantity); /// < добавить сумму в паевый фонд
  [[eosio::action]] void subcirculate(eosio::name coopname, eosio::name contract, eosio::asset quantity); /// < списать сумму из паевого фонда

    //фонды накопления
  [[eosio::action]] void addaccum( eosio::name coopname, eosio::name contract, uint64_t fund_id, eosio::asset quantity);
  [[eosio::action]] void subaccum( eosio::name coopname, eosio::name contract, uint64_t fund_id, eosio::asset quantity);
      //списать можно только с помощью вызова метода withdraw смарт-контракта

    //фонды списания
  [[eosio::action]] void addexpense( eosio::name coopname, eosio::name contract, uint64_t fund_id, eosio::asset quantity);
  ...
  [[eosio::action]] void spreadamount( eosio::name coopname, eosio::name contract, eosio::asset quantity); /// < распределить членские взносы по фондам накопления, положив остаток в фондовый кошелёк для дальнейшего списания
  ...
  [[eosio::action]] void authorize(eosio::name coopname, eosio::name type, uint64_t secondary_id);

  //используется авторизованными участником для подтверждения совершения выплаты
  [[eosio::action]] void complete(eosio::name coopname, eosio::name username, eosio::name type, uint64_t secondary_id);
